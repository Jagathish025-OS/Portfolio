...
const FRONTEND_BUILD = 'V16.1 · 2026-09-20';
...
// ---------------- GENERATOR MODES + AI BASE LAB V16 ----------------
...
function blueprintToSvg(b){
  const n=Number(b.gridSize)||44,cell=16,w=n*cell,h=n*cell;
  const fills={core:'#b05aa6',defense:'#4f7aa5',wall:'#a17a4d',resource:'#4e805e',support:'#607a8a',trap:'#b95b5b',decor:'#6c5a8c'};
  const esc=(v)=>String(v??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\"/g,'&quot;');
  let s=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}"><rect width="100%" height="100%" fill="#0b1420"/>`;
  for(let i=0;i<=n;i++)s+=`<path d="M${i*cell} 0V${h} M0 ${i*cell}H${w}" stroke="#ffffff" stroke-opacity=".035"/>`;
  for(const p of b.placements||[]){const fill=fills[p.category]||'#6b7280';const x=p.x*cell,y=p.y*cell,pw=p.w*cell,ph=p.h*cell;const label=esc(p.icon||'');const name=esc(p.label||p.id||'');s+=`<g><rect x="${x+1}" y="${y+1}" width="${Math.max(2,pw-2)}" height="${Math.max(2,ph-2)}" rx="3" fill="${fill}" stroke="#ffffff" stroke-opacity=".22"/><text x="${x+pw/2}" y="${y+ph/2+4}" text-anchor="middle" font-size="${Math.min(16,Math.max(8,Math.min(pw,ph)*.55))}">${label}</text>${pw>=32&&ph>=32?`<text x="${x+pw/2}" y="${y+ph-3}" text-anchor="middle" font-size="7" fill="#fff" fill-opacity=".75">${name}</text>`:''}</g>`;}
  return s+'</svg>';
}
