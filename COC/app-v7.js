...
const FRONTEND_BUILD = 'V12 · 2026-09-20';
...
// ---------------- GENERATOR MODES + AI BASE LAB V12 ----------------
...
async function buildCreativeIdea(idea) {
  ...
  $('generateBase').disabled=true; $('generateBase').textContent='🧩 Building…'; $('creativeBlueprintResult').hidden=true;
  try {
    ...
    setStatus(data.generationMode==='ai-blueprint' ? `AI designed ${idea.title} for TH${level}. Blueprint rendered and structurally checked.` : `AI blueprint provider unavailable · ${idea.title} rendered with verified server fallback.`,'success');
  } catch(error){ setStatus(error.message||'AI blueprint generation failed.','error'); }
  finally { updateBaseGenerateAvailability(); }
}
...
function renderCreativeBlueprint(payload) {
  ...
  setText('blueprintSummary',b.idea?.concept || b.prompt || 'AI creative blueprint generated.');
  const canvas=$('blueprintCanvas'); canvas.innerHTML='';
  const map=new Map((b.placements||[]).map(p=>[`${p.x},${p.y}`,p]));
  for(let y=0;y<b.gridSize;y++) for(let x=0;x<b.gridSize;x++){const cell=document.createElement('div');cell.className='blueprint-cell';const p=map.get(`${x},${y}`);if(p)cell.classList.add(p.category||'decor');cell.title=p?`${p.id} @ ${x},${y}`:`${x},${y}`;canvas.appendChild(cell);}
  setText('blueprintMeta',`${payload.strategySource==='AI'?'AI-designed':'Verified fallback'} · Structural validation: ${b.validation?.ok?'PASS':'FAIL'} · ${b.placements?.length||0} placed cells. ${b.note}`);
