...
const FRONTEND_BUILD = 'V5 · 2026-09-20';
...
async function loadLocalSnapshot(level) {
  ...
  return {
    ...
    heroes: (th.heroMaxLevels || []).map(x => ({ name: x.hero, maxLevel: x.maxLevel })),
...
async function loadGameData(level) {
  ...
  setStatus(`TH${level} local verified dataset loaded. Checking Jagathish Backend…`, 'success');

  // Server data is authoritative when reachable. Local data is shown immediately
  // so a sleeping Render service can never leave the page blank.
  try {
    ...
    setStatus(`TH${level} local data is ready. Server data unavailable (${reason}).`, 'error');
  }
}
