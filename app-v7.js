...
const FRONTEND_BUILD = 'V10 · 2026-09-20';
...
// ---------------- GENERATOR MODES + BASE GENERATOR V10 ----------------
...
function setGeneratorMode(mode) {
  ...
  setText('generatorLede', isBase
    ? 'Select your Town Hall, choose what the base is for, and the server will return a verified community layout.'
...
function updateBaseGenerateAvailability() {
  ...
  button.title = allowed ? '' : 'Community base layouts start at Town Hall 4.';
  if (!allowed) {
    setText('basePurposeStatus', 'Community base layouts start at Town Hall 4.');
    $('basePurposeStatus').classList.add('error');
  } else {
    $('basePurposeStatus').classList.remove('error');
    setText('basePurposeStatus', `Selected purpose: ${selectedBasePurpose}.`);
  }
}
...
async function loadBasePurposeAvailability(level) {
  ...
  try {
    const data = await fetchJson(
      `${BACKEND_URL}/api/coc/base-catalog-status?townHall=${encodeURIComponent(level)}`,
    ...
    if (!available.includes(selectedBasePurpose)) {
      const fallback = available[0];
      if (fallback) selectBasePurpose(fallback);
    }
    updateBaseGenerateAvailability();
  } catch (error) {
    // Keep the choices usable; the generation endpoint performs the authoritative validation.
    document.querySelectorAll('.purpose-card').forEach(card => {
      card.disabled = false;
      card.classList.remove('base-purpose-unavailable');
      card.title = '';
    });
    updateBaseGenerateAvailability();
    if (activeGenerator === 'base') {
      setText('basePurposeStatus', `Catalog availability could not be checked: ${error.message}`);
      $('basePurposeStatus').classList.add('error');
    }
  }
}
